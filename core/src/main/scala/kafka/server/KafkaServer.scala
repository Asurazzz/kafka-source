  ...
  def startup() {
    try {
      ...
      if (canStartup) {
        ...
        // 初始化配额管理服务,对于每个producer或者consumer,可以对他们produce或者 consum的速度上限作出限制
        ...
        /* start log manager */
        // 创建日志管理组件，创建时会检查log目录下是否有.kafka_cleanshutdown文件，
        // 如果没有的话，broker进入RecoveringFrom UncleanShutdown 状态
